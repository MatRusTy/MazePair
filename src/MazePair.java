...
public class MazePair {
    public int shortestPath(char[][] maze) {
        ...
        ArrayList<Vertex> rVertices = new ArrayList<>();

        // Create vertices
        for(int i = 1; i < height-1; i++){
            for(int j = 1; j < width-1; j++){
                if(maze[i][j] == '.'){
                    vertices[i][j] = new Vertex(0);
                    allVertices.add(vertices[i][j]);
                } else if(maze[i][j] == 'r') {
                    vertices[i][j] = new Vertex(1);
                    allVertices.add(vertices[i][j]);
                    rVertices.add(vertices[i][j]);
                } else if(maze[i][j] == 'b') {
                    vertices[i][j] = new Vertex(2);
                    allVertices.add(vertices[i][j]);
                }
            }
        }

        // Adjacent vertices are found
        for(int i = 1; i < height-1; i++){
            for(int j = 1; j < width-1; j++){
                if(maze[i][j] == '.' || maze[i][j] =='r' || maze[i][j] == 'b'){
                    Vertex u = vertices[i][j];
                    if(maze[i-1][j] =='.'|| maze[i-1][j] =='r' || maze[i-1][j] == 'b'){
                        u.getAdj().add(vertices[i-1][j]);
                    }
                    if(maze[i+1][j] =='.'|| maze[i+1][j] =='r' || maze[i+1][j] == 'b'){
                        u.getAdj().add(vertices[i+1][j]);
                    }
                    if(maze[i][j-1] =='.'|| maze[i][j-1] =='r' || maze[i][j-1] == 'b'){
                        u.getAdj().add(vertices[i][j-1]);
                    }
                    if(maze[i][j+1] =='.'|| maze[i][j+1] =='r' || maze[i][j+1] == 'b'){
                        u.getAdj().add(vertices[i][j+1]);
                    }
        ...
        for(Vertex startNode : rVertices){
            ...
            while(!queue.isEmpty()){
                ...
                for(Vertex v : u.getAdj()) {
                    if (v.getColor().equals("white") && v.getType() != 1) {
                        v.setColor("gray");
                        v.setD(u.getD() + 1);
                        queue.add(v);
                        if(v.getType() == 2){
                            shortestPath = Math.min(shortestPath, v.getD());
                        }
                    }
                }
                u.setColor("black");
            }
            if(shortestPath < bestSoFar){
                bestSoFar = shortestPath;
            }
        }
        return bestSoFar;
    }
